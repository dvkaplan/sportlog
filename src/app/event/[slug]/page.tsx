...
function FighterName({ name }: { name: string }) {
  ...
  return slug ? (
    <Link href={`/fighter/${slug}`} onClick={(e) => e.stopPropagation()} className="hover:text-accent hover:underline underline-offset-4">
      {name}
    </Link>
  ) : (
    <span>{name}</span>
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-3xl px-6 py-12">
        <BackLink />
        <div className="mt-4 text-xs font-semibold uppercase tracking-widest text-accent">Event</div>
          ...
          {event.fights.map((f) => {
           ...
           return (
              <div
                ...
                className="block cursor-pointer rounded-xl border border-zinc-800 bg-zinc-900 p-4 transition hover:border-accent"
              >
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <div className="font-medium">
                    <FighterName name={na ?? ""} /> <span className="text-zinc-500">vs</span> <FighterName name={nb ?? ""} />
                  </div>
                  {f.weightclass && <span className="text-xs text-accent">{f.weightclass}</span>}
                </div>
