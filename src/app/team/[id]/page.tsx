...
<div className="mt-10 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
          ...
          {fanLists.length > 0 ? (
            ...
            <p className="mt-2 text-sm text-zinc-500">
              ...
              <Link href="/lists/new" className="text-accent hover:underline">Start one</Link> — pick it from the dropdown so it links here.
            </p>
          )}
        </div>
        ...
        {(() => {
          ...
          const card = (p: (typeof players)[number]) => (
            <Link key={p.idPlayer} href={`/player/${p.idPlayer}`}
              className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-sm transition hover:border-accent">
          ...
          const staffRow = (role: string, name: string, href?: string) => (
            <div key={role + name} className="flex items-start justify-between gap-4 rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-2.5 text-sm">
              ...
              {href ? (
                <Link href={href} className="min-w-0 text-right font-medium text-zinc-100 hover:text-accent hover:underline underline-offset-4">{name}</Link>
              ) : (
            ...
            <>
              ...
              {groups.length > 4 && (
                <button onClick={() => setShowAll(!showAll)}
                  className="mt-4 rounded-lg border border-zinc-700 px-4 py-2 text-sm transition hover:border-accent">
