    ...
    <div className="mt-8 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button onClick={() => setOpen(!open)}
          className="rounded-lg border border-zinc-700 px-4 py-2 text-sm transition hover:border-accent">
          ...
          {userId ? (
            <>
              ...
              <textarea value={review} onChange={(e) => { setReview(e.target.value); setSaved(false); }}
                ...
                className="mt-4 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-3 text-sm outline-none focus:border-accent" />
              <button onClick={save} disabled={busy}
                className="mt-3 w-full rounded-lg bg-accent py-2.5 font-semibold text-zinc-950 transition hover:bg-accent disabled:opacity-50">
            ...
            <div className="text-center">
              ...
              <Link href="/auth" className="mt-3 inline-block rounded-lg bg-accent px-5 py-2 text-sm font-semibold text-zinc-950 transition hover:bg-accent">Sign in</Link>
            </div>
          )}
        </div>
      )}
