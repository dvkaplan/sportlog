...
export default function EntityRatingBox({
  entityType,
  entityId,
  entityName,
}: {
  ...
  async function save() {
    ...
    const { error } = await supabase.from("entity_ratings").upsert(
      { user_id: userId, entity_type: entityType, entity_id: entityId, rating, review, updated_at: new Date().toISOString() },
  ...
  return (
    <>
      <div className="mt-8 flex items-center gap-6 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
        <div>
        ...
        {userId ? (
          <>
            ...
            <textarea
              ...
              placeholder={`Your take on ${entityName}…`}
              rows={4}
          ...
          <div className="text-center">
            <p className="text-zinc-300">Sign in to rate {entityName}.</p>
            <Link href="/auth" className="mt-4 inline-block rounded-lg bg-accent px-6 py-2.5 font-semibold text-zinc-950 transition hover:bg-accent">
              Sign in
            </Link>
          </div>
        )}
