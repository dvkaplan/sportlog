...
type Cat = {
  name: string;
  labels: string[];
  seasons: { season: string; team: string; stats: string[] }[];
  totals: string[];
};

export default function PlayerStatsNFL({ espnId }: { espnId: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const j = await fetch(`/api/nfl-stats?id=${espnId}`).then((r) => r.json());
