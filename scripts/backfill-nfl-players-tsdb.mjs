...
const missing = JSON.parse(readFileSync("src/lib/nfl-missing-players.json", "utf8"));
...
for (const m of missing) {
  ...
  const hit = (list ?? []).find((p) => (p.strSport ?? "") === "American Football" &&
    (norm(p.strPlayer) === norm(m.name) || bare(p.strPlayer) === bare(m.name)));
  if (hit) {
    players.push({
      idPlayer: hit.idPlayer, strPlayer: m.name, idTeam: hit.idTeam ?? "",
      strTeam: hit.strTeam ?? "_Retired", strLeague: "NFL", strSport: "American Football",
...
writeFileSync("src/lib/players.json", JSON.stringify(players));
console.log(`\nNFL rescue done: added ${added} of ${missing.length}.`);