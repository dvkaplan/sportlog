...
for (const g of games) {
  ...
  out[g.id] = { event: m.event, weightclass: m.weightclass, method: m.method, round: m.round, time: m.time, format: m.format, referee: m.referee, details: m.details, fighters: [{ name: m.a, ...(pack(m.a) ? { stats: pack(m.a) } : {}) }, { name: m.b, ...(pack(m.b) ? { stats: pack(m.b) } : {}) }] };
  matched++;
}
